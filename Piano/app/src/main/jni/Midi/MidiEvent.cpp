...
MidiEvent MidiEvent::getNextEventFromStream(IKSStream *stream, uint8_t runningStatus)
{
    ...
    do
    {
        ...
        {
           KSLOGE(LOGTAG,"read delta time - stream error");//shouldn't happen in a good midifile
            break;//return  =0 ev
        }
    ...
    {
       KSLOGE(LOGTAG,"stream error status");
    ...
    switch (event.getType())
    {
        case MIDIEVENT_SYS_EX:
           // KSLOGD(LOGTAG,"readSYSEX");
            bRead = event.readSysEX(stream);
            break;
        case MIDIEVENT_META:
          //  KSLOGD(LOGTAG,"read meta");
            bRead = event.readMeta(stream);
            break;
        default:
           // KSLOGD(LOGTAG,"read default");
...
bool MidiEvent::readMIDI(IKSStream *stream)
{
    ...
    switch (getType())
    {
        ...
        case MIDIEVENT_PROGRAM_CHANGE:
            ...
            break;
        default:
           KSLOGE(LOGTAG,"readMIDI type unregistered");
            return false;
...
bool MidiEvent::readSysEX(IKSStream *stream)
{
    ...
    {
       KSLOGE(LOGTAG,"read sysex stream failed");
        return false;
    }
    //F7 inside the buffer?

    KSLOGD(LOGTAG,"the sysex data %s",data);
    return true;
}
...
bool MidiEvent::readMeta(IKSStream *stream)
{
    ...
    {
       KSLOGE(LOGTAG,"stream readMeta failed");
        return false;
    }
    ...
    do
    {
        ...
        {
           KSLOGE(LOGTAG,"read meta length- stream error");//shouldn't happen in a good midifile
            return false;
        }
    ...
    {
       KSLOGE(LOGTAG,"read metaBuffer failed lenght %lu",length);
        delete[] data;
        return false;
    }

    KSLOGD(LOGTAG,"meta data %s",data);
    switch(metaType)
    {
        ...
        { //stored in 3 bytes;microseconds per quarter note
            if (length < 3) {
               KSLOGE(LOGTAG, "midi parse error set tempo");
                delete[] data;
                return false;
            }
            // tempo = 0;
        ...
        case MIDIMETAEVENT_META_TAG:
            break;
        default:
           KSLOGE(LOGTAG,"midi meta data unregistered");
    }
...
KeyStateInfo MidiEvent::getKeyState()
{
    ...
    if((event == MIDIEVENT_NOTE_ON) || (event == MIDIEVENT_NOTE_OFF))
    {
        ...
        {
           KSLOGE(LOGTAG,"Please implement : the current midi PianoCore key name doesn't support below a0 :");
        }
    }
    return keyState;
}
