...
bool MidiFile::decode(IKSStream *input,std::string filename)
{
    ...
    KSLOGD(LOGTAG,"isLitteIndian %d",bLittleEndian);
    bOpened  = false;
    if(!input)
    {
       KSLOGE(LOGTAG,"input file not open");
        return bOpened;
    }
    ...
    if(input->read(chunkId, 4) > 0 && input->read(chunkSize, 4) > 0)
    {
        ...
        if(chunkId[0] == 'M' && chunkId[1] == 'T' && chunkId[2] == 'h' && chunkId[3] == 'd' && (*(int32_t *)chunkSize) == 6)//maybe >6 also ok>
        {
            ...
            {
               KSLOGE(LOGTAG, "invalid Midi format");
                return false;
            }
            ...
            {
               KSLOGE(LOGTAG,"error 0 format can have only one track");
                return false;
            }
        ...
        {
           KSLOGE(LOGTAG,"%s might not be midifile",filename.c_str());
            return false;
        }
        ...
        {
            KSLOGV(LOGTAG,"reading midi track %u",trkNo);
           if( input->read(chunkId,4) == 4 )//TODO
           {
               if(chunkId[0] == 'M' && chunkId[1] == 'T' && chunkId[2] == 'r' && chunkId[3] == 'k' )
               {
                   ...
                   do {
                       ...
                       {
                           KSLOGD(LOGTAG,"tempo %d",event.getTempo());
                           trackInfos[trkNo].tempo = event.getTempo();
                       }
                   } while (!event.isEndOfTrack());


                   KSLOGD(LOGTAG,"MidiEvent count %ld",cnt);
            ...
            {
               KSLOGE(LOGTAG,"error reading track");
                return false;
            }
    ...
    {
       KSLOGE(LOGTAG,"Midi file read error");
    }
...
{
    KSLOGV(LOGTAG,"format %d trackCnt %u tick format %s - tickRate %lf",midiFormat,trackCnt,tickRate,bUsingFPS ? "FFPS":"tickPerQuarterNote",tickRate);
}

void MidiTrackFileInfo::print() const
{
    KSLOGV(LOGTAG,"track byte size %d ,track file offset %ld",sizeBytes,fileOffset);
}
